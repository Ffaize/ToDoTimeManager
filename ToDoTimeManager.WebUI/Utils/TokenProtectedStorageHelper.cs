...
public static class TokenProtectedStorageHelper
{
    ...
    {
        ProtectedBrowserStorageResult<string> result = await protectedLocalStorage.GetAsync<string>(nameof(TokenModel));

        return result is { Success: true, Value: not null }
            ? JsonSerializer.Deserialize<TokenModel>(result.Value)
            : null;
    }
