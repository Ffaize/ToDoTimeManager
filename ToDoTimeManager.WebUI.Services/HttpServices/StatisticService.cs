using ToDoTimeManager.Shared.DTOs.MainPageStatistic;
...
public class StatisticService : BaseHttpService
{
    ...
    public StatisticService(IHttpClientFactory httpClientFactory, ILogger<StatisticService> logger) : base(
        httpClientFactory)
    {
    ...
    public async Task<MainPageStatisticModel> GetMainPageStatistic(MainPageStatisticRequestDto filter)
    {
        ...
        {
            _logger.LogError(e,
                "An error occurred while fetching main page statistics for user {UserId} with filter {TimeFilter}",
                filter.UserId, filter.TimeFilter);
